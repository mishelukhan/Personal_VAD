...
def get_enroll(dataset):
    ...
    for name, (_, g_t) in enumerate(dataset):
        ...
        window = Segment(segments[0].start, segments[0].start + win_len)
        intersect = []
        for seg in segments:
...
def get_testpart(dataset):
    ...
    for name, (_, g_t) in enumerate(dataset):
        ...
        while intersect:
            ...
            condition = max_dur / min_dur < 5 and max_end - min_start > 40 and len(dict_enroll) >= 3
            if condition:
...
test_data = BenchmarkDataset(DATA_ROOT, GROUND_TRUTH)
enrolls = get_enroll(test_data)
test_dict = get_testpart(test_data)
